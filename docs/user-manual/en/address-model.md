...
<address-settings>
   <address-setting match="order.foo">
      ...
      <enable-ingress-time>false</enable-ingress-time>
   </address-setting>
</address-settings>
```
...
`enable-ingress-time` determines whether or not the broker will add its time to 
messages sent to the matching address. When `true` the exact behavior will depend 
on the specific protocol in use. For AMQP messages the broker will add a `long` 
*message annotation* named `x-opt-ingress-time`. For core messages (used by the 
core and OpenWire protocols) the broker will add a long property named
`_AMQ_INGRESS_TIME`. For STOMP messages the broker will add a frame header named
`ingress-time`. The value will be the number of milliseconds since the
