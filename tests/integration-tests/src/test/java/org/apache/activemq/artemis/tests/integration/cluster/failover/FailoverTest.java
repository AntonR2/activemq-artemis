...
public class FailoverTest extends FailoverTestBase {
   ...
   private void doSimpleSendAfterFailover(final boolean durable, final boolean temporary) throws Exception {
      ...
      if (temporary) {
         session.createTemporaryQueue(new QueueConfiguration(FailoverTestBase.ADDRESS).setDurable(durable));
      } else {
         session.createQueue(new QueueConfiguration(FailoverTestBase.ADDRESS).setDurable(durable));
      }
