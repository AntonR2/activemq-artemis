...
public class ClusteredJMSTempQueueTest extends ClusterTestBase {
   ...
   public void testDuplicateCacheCleanupForTempQueues() throws Exception {
      ...
      for (int j = 0; j < 10; j++) {
         try (Connection connection = cf.createConnection("guest", "guest")) {
            SimpleRequestServer server = new SimpleRequestServer().start();
...
class SimpleRequestServer implements MessageListener {
   ...
   public SimpleRequestServer start() throws Exception {
      Queue requestQueue = ActiveMQJMSClient.createQueue("exampleQueue");
      ...
      connection.start();
      replyProducer = session1.createProducer(null);
      requestConsumer = session1.createConsumer(requestQueue);
   ...
   public void onMessage(final javax.jms.Message request) {
      try {
         ...
         TextMessage replyMessage = session1.createTextMessage("A reply message");
