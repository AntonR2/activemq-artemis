...
public class QueueConfiguration implements Serializable {
   ...
   public static QueueConfiguration fromJSON(String jsonString) {
      ...
      // id is not set as it will be generated automatically when the queue is created

      return new QueueConfiguration(json.getString(NAME))
         .setAddress(json.keySet().contains(ADDRESS) ? json.getString(ADDRESS) : null)
         .setRoutingType(json.keySet().contains(ROUTING_TYPE) ? RoutingType.valueOf(json.getString(ROUTING_TYPE)) : null)
         .setFilterString(json.keySet().contains(FILTER_STRING) ? json.getString(FILTER_STRING) : null)
         .setDurable(json.keySet().contains(DURABLE) ? json.getBoolean(DURABLE) : null)
         .setUser(json.keySet().contains(USER) ? json.getString(USER) : null)
         .setMaxConsumers(json.keySet().contains(MAX_CONSUMERS) ? json.getInt(MAX_CONSUMERS) : null)
         .setExclusive(json.keySet().contains(EXCLUSIVE) ? json.getBoolean(EXCLUSIVE) : null)
         .setGroupRebalance(json.keySet().contains(GROUP_REBALANCE) ? json.getBoolean(GROUP_REBALANCE) : null)
         .setGroupBuckets(json.keySet().contains(GROUP_BUCKETS) ? json.getInt(GROUP_BUCKETS) : null)
         .setGroupFirstKey(json.keySet().contains(GROUP_FIRST_KEY) ? json.getString(GROUP_FIRST_KEY) : null)
         .setLastValue(json.keySet().contains(LAST_VALUE) ? json.getBoolean(LAST_VALUE) : null)
         .setLastValueKey(json.keySet().contains(LAST_VALUE_KEY) ? json.getString(LAST_VALUE_KEY) : null)
         .setNonDestructive(json.keySet().contains(NON_DESTRUCTIVE) ? json.getBoolean(NON_DESTRUCTIVE) : null)
         .setPurgeOnNoConsumers(json.keySet().contains(PURGE_ON_NO_CONSUMERS) ? json.getBoolean(PURGE_ON_NO_CONSUMERS) : null)
         .setConsumersBeforeDispatch(json.keySet().contains(CONSUMERS_BEFORE_DISPATCH) ? json.getInt(CONSUMERS_BEFORE_DISPATCH) : null)
         .setDelayBeforeDispatch(json.keySet().contains(DELAY_BEFORE_DISPATCH) ? json.getJsonNumber(DELAY_BEFORE_DISPATCH).longValue() : null)
         .setConsumerPriority(json.keySet().contains(CONSUMER_PRIORITY) ? json.getInt(CONSUMER_PRIORITY) : null)
         .setAutoDelete(json.keySet().contains(AUTO_DELETE) ? json.getBoolean(AUTO_DELETE) : null)
         .setAutoDeleteDelay(json.keySet().contains(AUTO_DELETE_DELAY) ? json.getJsonNumber(AUTO_DELETE_DELAY).longValue() : null)
         .setAutoDeleteMessageCount(json.keySet().contains(AUTO_DELETE_MESSAGE_COUNT) ? json.getJsonNumber(AUTO_DELETE_MESSAGE_COUNT).longValue() : null)
         .setRingSize(json.keySet().contains(RING_SIZE) ? json.getJsonNumber(RING_SIZE).longValue() : null)
         .setConfigurationManaged(json.keySet().contains(CONFIGURATION_MANAGED) ? json.getBoolean(CONFIGURATION_MANAGED) : null)
         .setTemporary(json.keySet().contains(TEMPORARY) ? json.getBoolean(TEMPORARY) : null)
         .setAutoCreateAddress(json.keySet().contains(AUTO_CREATE_ADDRESS) ? json.getBoolean(AUTO_CREATE_ADDRESS) : null)
         .setInternal(json.keySet().contains(INTERNAL) ? json.getBoolean(INTERNAL) : null)
         .setTransient(json.keySet().contains(TRANSIENT) ? json.getBoolean(TRANSIENT) : null)
         .setAutoCreated(json.keySet().contains(AUTO_CREATED) ? json.getBoolean(AUTO_CREATED) : null);
   }
