...
public class ActiveMQSession implements QueueSession, TopicSession {
   ...
   public TemporaryQueue createTemporaryQueue() throws JMSException {
      ...
      try {
         ...
         session.createTemporaryQueue(new QueueConfiguration(simpleAddress).setRoutingType(RoutingType.ANYCAST));
   ...
   public TemporaryTopic createTemporaryTopic() throws JMSException {
      ...
      try {
         ...
         session.createTemporaryQueue(new QueueConfiguration(simpleAddress).setAddress(simpleAddress).setFilterString(ActiveMQSession.REJECTING_FILTER));
   ...
   void createTemporaryQueue(ActiveMQDestination destination, RoutingType routingType, SimpleString queueName, SimpleString filter, ClientSession.AddressQuery addressQuery) throws ActiveMQException {
      ...
      session.createTemporaryQueue(queueConfiguration.setName(queueName).setAddress(destination.getAddress()));
   }
   ...
   private void setRequiredQueueConfigurationIfNotSet(QueueConfiguration queueConfiguration, ClientSession.AddressQuery addressQuery, RoutingType routingType, SimpleString filter, boolean durable) {
      ...
      if (queueConfiguration.isDurable() == null) {
